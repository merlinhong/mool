...
const { resolvePluginId,isOfficialPlugin } = require("./util/resolvePlugin");
async function add(pluginToAdd, options = {}, context = process.cwd()) {
  if (!(await confirmIfGitDirty(context))) {
    return;
  }

  // for `mool add` command in 3.x projects
  const servicePkg = loadModule("@mooljs/cli-service/package.json", context);
  if (servicePkg && semver.satisfies(servicePkg.version, "3.x")) {
    // special internal "plugins"
    if (/^(@mooljs\/)?router$/.test(pluginToAdd)) {
  ...
  log(
    `${chalk.green("✔")}  Successfully installed plugin: ${chalk.cyan(packageName)}`,
  );
  log();
