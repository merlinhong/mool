import {
  ...
  UseProviderOptions,
  RuntimeConfig,
} from "./type"; // 假设type.ts和hooks文件在同一目录
...
function findRouteByPath(
  routes: IMenuRoutes[],
  targetPath: string,
): [IMenuRoutes | null, IMenuRoutes] | null {
...
function checkPermision(options: CheckPermissionOptions) {
  ...
  if (res) {
    ...
    const noPermission =
      (hasNoPermision(access, parentRoutes?.meta?.access) ||
        hasNoPermision(access, route.meta?.access)) &&
      !exclude.some((_) => to.path.includes(_));
    return noPermission && to.path !== "/403";
  } else {
    return false;
  }
}
export const getAccess = async (options: {
  config: RuntimeConfig;
  access: UseProviderOptions["access"];
}) => {
  const {
  ...
  return { access: accessConfig };
};
export const setupAccess = (app: App, options: UseProviderOptions) => {
  ...
  router?.addRoute({
    ...
    component:
      layout.unAccessible ??
      (() => import("@mooljs/plugin-layout/src/layouts/403.vue")),
  });
  router?.beforeEach((to, from, next) => {
    const exclude = routes
      .filter((_) => _.meta?.layout === false)
      ?.map((_) => _.path);
